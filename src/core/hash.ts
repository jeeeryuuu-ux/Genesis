...
/**
 * Hashes a string deterministically with an optional seed modifier.
 */
export function hashString(str: string, seed: number = 0): Seed {
  let h = (seed >>> 0) ^ 0x9e3779b9;
  for (let i = 0; i < str.length; i++) {
    h = Math.imul(h ^ str.charCodeAt(i), 0x5bd1e995) >>> 0;
    h = (h ^ (h >>> 15)) >>> 0;
  }
  return hash32(h, seed);
}

/**
 * Deterministically mixes a parent seed, a string or numeric key, and a domain salt.
 */
export function hash32WithSalt(
  parentSeed: number,
  key: string | number,
  domainSalt: number
): Seed {
  const keyHash = typeof key === 'string' ? hashString(key, parentSeed) : hash32(key, parentSeed);
  return hash32(domainSalt, keyHash);
}
