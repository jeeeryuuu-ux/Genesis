...
import { BiosphereModal } from './components/BiosphereModal.js';
import { CivilizationModal } from './components/CivilizationModal.js';
...
export default function App() {
  ...
  const [inspectingBiospherePlanet, setInspectingBiospherePlanet] = useState<Planet | null>(null);
  const [inspectingCivilizationPlanet, setInspectingCivilizationPlanet] = useState<Planet | null>(null);

  // Authoritative Biological Queries (Lazy & Seed-Hierarchical)
  const selectedPlanetBiosphereSummary = useMemo(() => {
    if (!selectedPlanet) return undefined;
    return simulationEngine.getBiosphereSummary(selectedPlanet);
  }, [selectedPlanet, currentYear, simulationEngine]);

  const inspectingBiosphere = useMemo(() => {
    if (!inspectingBiospherePlanet) return undefined;
    return simulationEngine.getBiosphere(inspectingBiospherePlanet);
  }, [inspectingBiospherePlanet, currentYear, simulationEngine]);

  // Authoritative Civilization Queries (Lazy & Seed-Hierarchical)
  const selectedPlanetCivilizationSummary = useMemo(() => {
    if (!selectedPlanet) return undefined;
    return simulationEngine.getCivilizationSummary(selectedPlanet);
  }, [selectedPlanet, currentYear, simulationEngine]);

  const inspectingCivilizations = useMemo(() => {
    if (!inspectingCivilizationPlanet) return [];
    return simulationEngine.getCivilizations(inspectingCivilizationPlanet);
  }, [inspectingCivilizationPlanet, currentYear, simulationEngine]);

  const inspectingCivilizationHistory = useMemo(() => {
    if (!inspectingCivilizationPlanet) return [];
    return simulationEngine.getCivilizationHistory(inspectingCivilizationPlanet.id);
  }, [inspectingCivilizationPlanet, simulationEngine]);
  ...
  return (
    <main className="relative w-screen h-screen overflow-hidden bg-slate-950 font-sans select-none">
      ...
      >
        ...
        <div className="flex items-center gap-2.5 pointer-events-auto">
          <div className="flex items-center gap-2 px-3 py-1.5 bg-slate-950/80 backdrop-blur-md border border-slate-800 rounded-lg shadow-xl">
            ...
            <span className="text-[10px] font-mono px-1.5 py-0.5 bg-emerald-950/80 text-emerald-400 border border-emerald-800/40 rounded">
              v0.6.0
            </span>
          </div>
      ...
      <InfoPanel
        ...
        biosphereSummary={selectedPlanetBiosphereSummary}
        civilizationSummary={selectedPlanetCivilizationSummary}
        currentYear={currentYear}
        onEnterGalaxy={handleEnterGalaxy}
        onEnterSystem={handleEnterSystem}
        onFocusPlanet={handleFocusPlanet}
        onInspectBiosphere={(planet) => setInspectingBiospherePlanet(planet)}
        onInspectCivilization={(planet) => setInspectingCivilizationPlanet(planet)}
      />
      ...
      {/* 10. Biological Genesis & Species Inspector Modal */}
      {inspectingBiospherePlanet && inspectingBiosphere && (
        <BiosphereModal
          isOpen={Boolean(inspectingBiospherePlanet)}
          onClose={() => setInspectingBiospherePlanet(null)}
          planet={inspectingBiospherePlanet}
          biosphere={inspectingBiosphere}
          currentYear={currentYear}
        />
      )}

      {/* 11. Civilization Genesis & Technological Evolution Inspector Modal */}
      {inspectingCivilizationPlanet && (
        <CivilizationModal
          isOpen={Boolean(inspectingCivilizationPlanet)}
          onClose={() => setInspectingCivilizationPlanet(null)}
          planet={inspectingCivilizationPlanet}
          civilizations={inspectingCivilizations}
          historicalRecords={inspectingCivilizationHistory}
          currentYear={currentYear}
        />
      )}
    </main>
